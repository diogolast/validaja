...
class BoletoSchema(BaseModel):
    ...
    nome_beneficiario: str = Field(
        description="Nome completo ou Razão Social do beneficiário (cedente) do boleto."
    )
    documento_beneficiario: str = Field(
        description="Número do CNPJ ou CPF do beneficiário (cedente)."
    )
    agencia_codigo_cedente: Optional[str] = Field(
        default=None, description="Número da agência e código do cedente/beneficiário."
    )
    endereco_beneficiario: Optional[str] = Field(
        default=None, description="Endereço completo do beneficiário (cedente)."
    )

    # Pagador (Sacado)
    nome_pagador: Optional[str] = Field(
        default=None,
        description="Nome completo ou Razão Social do pagador (sacado) do boleto.",
    )
    documento_pagador: Optional[str] = Field(
        default=None, description="Número do CNPJ ou CPF do pagador (sacado)."
    )
    endereco_pagador: Optional[str] = Field(
        default=None, description="Endereço completo do pagador (sacado)."
    )

    # Dados do Boleto
    codigo_banco_emissor: str = Field(
        description="Código do banco emissor do boleto (3 dígitos)."
    )
    nome_banco_emissor: Optional[str] = Field(
        default=None, description="Nome do banco emissor do boleto."
    )
    linha_digitavel: str = Field(description="Linha digitável completa do boleto.")
    codigo_barras_numerico: Optional[str] = Field(
        default=None, description="Código de barras numérico (44 dígitos)."
    )
    nosso_numero: Optional[str] = Field(
        default=None, description="Nosso Número do boleto."
    )
    numero_documento_boleto: Optional[str] = Field(
        default=None, description="Número do documento/boleto."
    )
    data_vencimento: str = Field(
        description="Data de vencimento no formato DD/MM/AAAA."
    )
    data_documento: Optional[str] = Field(
        default=None, description="Data de emissão no formato DD/MM/AAAA."
    )
    valor_documento: float = Field(description="Valor do documento/boleto.")
    valor_cobrado: Optional[float] = Field(
        default=None, description="Valor final a ser cobrado."
    )

    # Campos adicionais
    especie_doc: Optional[str] = Field(
        default=None, description="Espécie do documento."
    )
    local_pagamento: Optional[str] = Field(
        default=None, description="Local de pagamento."
    )
    demonstrativo: List[str] = Field(
        default_factory=list, description="Linhas do demonstrativo/detalhamento."
    )
    instrucoes_caixa: List[str] = Field(
        default_factory=list, description="Instruções para o caixa."
    )
...
class AnaliseComparacaoSchema(BaseModel):
    ...
    eh_fraudulento: bool = Field(
        description="True se o boleto é potencialmente fraudulento, False se parece legítimo"
    )
    ...
    diferencas_encontradas: List[str] = Field(
        description="Lista das principais diferenças encontradas"
    )
    pontos_suspeitos: List[str] = Field(
        description="Lista de pontos suspeitos identificados"
    )
    recomendacao: str = Field(
        description="Recomendação final (PAGAR, NAO_PAGAR, VERIFICAR_MANUALMENTE)"
    )


def criar_prompt_comparacao(
    boletos_referencia: List[Dict], boleto_analise: Dict
) -> str:
...
def extrair_dados_boleto(arquivo_pdf_bytes: bytes) -> Tuple[Dict, bool]:
    ...
    try:
        ...
        # Garante que o arquivo foi fechado antes de tentar usá-lo
        if temp_file_path and os.path.exists(temp_file_path):
            try:
                ...
                print(f"Erro ao processar o boleto: {e}")
                return {}, False
        ...
        if temp_file_path and os.path.exists(temp_file_path):
            try:
                ...
                print(
                    f"Aviso: Não foi possível remover o arquivo temporário {temp_file_path}: {e}"
                )
...
def normalizar_dados_comparacao(boleto: Dict) -> Dict:
    ...
    return {
        ...
        "documento_beneficiario": boleto.get("documento_beneficiario", "")
        .replace(".", "")
        .replace("/", "")
        .replace("-", "")
        .strip(),
        "codigo_banco_emissor": boleto.get("codigo_banco_emissor", "")
        .replace("-", "")
        .strip(),
        "agencia_codigo_cedente": boleto.get("agencia_codigo_cedente", "").strip(),
        "endereco_beneficiario": boleto.get("endereco_beneficiario", "")
        .strip()
        .upper(),
...
def analisar_fraude_boleto(
    ...
    try:
        ...
        if (
            ref_normalizado["nome_beneficiario"]
            != analise_normalizado["nome_beneficiario"]
        ):
            problemas_reais.append(
                f"Nome do beneficiário diferente: '{boleto_analise.get('nome_beneficiario')}' vs '{boletos_referencia[0].get('nome_beneficiario')}'"
            )
            pontos_suspeitos.append("Nome do beneficiário não confere")

        # Verifica documento do beneficiário (deve ser idêntico)
        if (
            ref_normalizado["documento_beneficiario"]
            != analise_normalizado["documento_beneficiario"]
        ):
            problemas_reais.append(
                f"Documento do beneficiário diferente: '{boleto_analise.get('documento_beneficiario')}' vs '{boletos_referencia[0].get('documento_beneficiario')}'"
            )
            pontos_suspeitos.append("CNPJ/CPF do beneficiário não confere")

        # Verifica banco emissor (deve ser o mesmo)
        if (
            ref_normalizado["codigo_banco_emissor"]
            != analise_normalizado["codigo_banco_emissor"]
        ):
            problemas_reais.append(
                f"Banco emissor diferente: '{boleto_analise.get('codigo_banco_emissor')}' vs '{boletos_referencia[0].get('codigo_banco_emissor')}'"
            )
            pontos_suspeitos.append("Banco emissor não confere")

        # Verifica agência/cedente (deve ser igual)
        if ref_normalizado.get("agencia_codigo_cedente") and analise_normalizado.get(
            "agencia_codigo_cedente"
        ):
            if (
                ref_normalizado["agencia_codigo_cedente"]
                != analise_normalizado["agencia_codigo_cedente"]
            ):
                problemas_reais.append(
                    f"Agência/cedente diferente: '{boleto_analise.get('agencia_codigo_cedente')}' vs '{boletos_referencia[0].get('agencia_codigo_cedente')}'"
                )
        ...
        nosso_nums_ref = [
            b.get("nosso_numero", "")
            for b in boletos_referencia
            if b.get("nosso_numero")
        ]
        ...
        if nosso_nums_ref and nosso_num_analise:
            ...
            if padrao_analise not in padroes_ref and len(set(padroes_ref)) == 1:
                anomalias_menores.append(
                    f"Formato do nosso número diferente: {padrao_analise} vs {padroes_ref[0]}"
                )
...
def processar_multiplos_boletos_referencia(
    arquivos_pdf: List[Tuple[bytes, str]],
